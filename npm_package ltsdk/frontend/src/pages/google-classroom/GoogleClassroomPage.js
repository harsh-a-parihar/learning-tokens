...
const GoogleClassroomPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null)
      try {
        // Request the course index from the SDK for Google Classroom
        const res = await lmsSdkClient.searchCourses('google-classroom', '')
        ...
        <div className="edx-container">
          <div className="edx-header-banner">
            <div className="edx-banner-content">
              <h1>Welcome to Google Classroom</h1>
              <p className="edx-subtitle">Manage your classrooms and connect with students</p>
            </div>
          </div>
              ...
              {loading ? (
                ...
                courses.map((course) => (
                  <div key={course.id || course.key} className="course-card">
                    <div className="card-top-accent" style={{ background: 'linear-gradient(90deg,#4285f4,#34a853)' }} />
                    ...
                    <div className="card-header">
                      <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div className="card-org">{course.section || course.organization || course.course_code}</div>
                      </div>
                      <div className="course-number">{course.enrollmentCode || course.number || ''}</div>
                    </div>
                    ...
                    <div className="card-footer">
                      ...
                      <div className="right-actions">
                        ...
                        <button className="btn btn-primary" onClick={() => navigate(`/course/google-classroom/${encodeURIComponent(course.id || course.key)}`)}>Manage</button>
                      </div>
                    </div>
                  </div>
                ))
